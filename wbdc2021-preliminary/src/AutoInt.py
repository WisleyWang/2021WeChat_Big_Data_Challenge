...
# 初赛待预测行为列表
ACTION_LIST = ["forward", "read_comment", "like", "click_avatar"]
...
if __name__ == "__main__":
    for idx,seed in enumerate([0,157,333,2048]):
            ...
            for fold_, (trn_idx, val_idx) in enumerate(folds.split(train[feature_names].values, train[target].values)):
                ...
                model = AutoInt(linear_feature_columns=linear_feature_columns, dnn_feature_columns=dnn_feature_columns,
                            ...
                            l2_reg_embedding=1e-1,
                                dnn_use_bn=True, dnn_dropout=0.5, device=device)
        ...
        # 保存提交文件
        torch.save(model.state_dict(), MODEL_PATH+'AutoInt_b_weights_{}.h5'.format(idx))
        submit[['userid', 'feedid', "read_comment", "like", "click_avatar", "forward"]].to_csv(SUBMIT_PATH+"submit_b_autoint_{}.csv".format(idx), index=False)
