...
class Model(nn.Module):
    def __init__(self,usr_data,feed_data,feed_embed,graph_emb):
       ...
       ,'manual_keyword_id1':16,'manual_tag_id1':16,'machine_keyword_id1':16
        ...
        self.dynami_dense=nn.Linear(60,64)
...
#         self.dnn=DNN(sum(user_dict.values())+128*3+64,(128,128),dropout_rate=0.1)
        self.mmoe=MMOELayer(sum(user_dict.values())+128*3+64, mmoe_hidden_dim=128,num_task=5,n_expert=5,expert_activation=None)
...
#         print(user_projections)
        ...
        logit_reg=self.reg_liner(outs[4])
        return logit_gnn1,logit_gnn2,logit_gnn3,logit_gnn4,logit_reg
...
#         print(batch_src)
        ...
        loss=criti(logits[0][:,0],batch_label[:,0])*0.8+criti(logits[1][:,0],batch_label[:,1])*0.8+        criti(logits[2][:,0],batch_label[:,2])*0.4+criti(logits[3][:,0],batch_label[:,3])*0.4+reg_criti(logits[4][:,0],batch_reg)*0.6
...
# 保存模型
torch.save(model,os.path.join(MODEL_PATH,'my_deep_v2_v1.pth'))

test_pred=test_pred_func(model)
test_a[PREDICT_LIST]=test_pred[:,:4]
sub=test_a[-len(test_b):][['userid','feedid']+PREDICT_LIST]

# 保存
sub.to_csv(os.path.join(SUBMMIT_PATH,'my_deep_v2_v1_b1'),index=False)
