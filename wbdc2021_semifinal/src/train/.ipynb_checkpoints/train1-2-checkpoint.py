...
graph_emb=np.concatenate([np.load(ROOT_PATH+'tmp/grap_allembedding32+day_sg2.npy'),np.load(ROOT_PATH+'tmp/grap_allembedding32+day_hs2.npy')],axis=1)
...
model = Model2(user_data,feed_data,textset
...
optimizer, scheduler = build_optimizer(model, train_steps, learning_rate=2.6e-3)
all_pred=[]
...
for epoch in range(epochs):
    ...
    for ind in tqdm(range(0,n_pos//batch_size+1)):
        ...
        batch_src=src[batch].cuda()
        batch_dst=dst[batch].cuda()
...
#  保存模型
torch.save(model, ROOT_PATH+'/model/model2.pth')
#---------------------------------------------------
