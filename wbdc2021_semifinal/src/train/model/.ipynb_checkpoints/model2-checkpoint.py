...
#         cross7=self.cross7(combine)
        ...
        logit_gnn5=self.liner5(torch.cat([outs[0],cross2],axis=-1))#+ffm2
        logit_gnn6=self.liner6(torch.cat([outs[2],cross3],axis=-1))#+ffm3
        logit_gnn7=self.liner7(torch.cat([outs[4],cross4],axis=-1))#+ffm4
...
class MMOELayer(nn.Module):
    def __init__(self, hidden_size, mmoe_hidden_dim=128,num_task=4,n_expert=3,expert_activation=None):
        ...
        for i in range(num_task):
            setattr(self, 'task_{}_dnn'.format(i+1),DNN(mmoe_hidden_dim,(128,128),dropout_rate=0.2,l2_reg=1e-4,seed=random.sample([1018,2020,2021,1024],1)[0],use_bn=True))
    def forward(self,x):
         # mmoe
