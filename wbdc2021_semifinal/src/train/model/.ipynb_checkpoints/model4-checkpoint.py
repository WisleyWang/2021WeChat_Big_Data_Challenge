...
class Model4(nn.Module):
    def __init__(self,user_data,feed_data,textset,feed_embed,graph_emb,user_graph):
        super().__init__()
       ...
       ,'manual_keyword_id1':16,'manual_tag_id1':16,'machine_keyword_id1':16
        ...
        self.text_liner=nn.Linear(32*6+512+64*2+64,256)
        ...
        self.user_graph= nn.Parameter(torch.from_numpy(user_graph).float(),requires_grad=False)
        
        self.cross1=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
        self.cross2=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
        self.cross3=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
        self.cross4=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
        self.cross5=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
        self.cross6=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
        self.cross7=CrossNetMix(sum(user_dict.values())+128*4,layer_num=4)
#         self.dnn=DNN(sum(user_dict.values())+128*3+64,(128,128),dropout_rate=0.1)
        self.mmoe=MMOELayer(sum(user_dict.values())+128*4, mmoe_hidden_dim=256,num_task=7,n_expert=7,expert_activation=None)

        self.liner1=nn.Linear(128+sum(user_dict.values())+128*4,1)
        self.liner2=nn.Linear(128+sum(user_dict.values())+128*4,1)
        self.liner3=nn.Linear(128+sum(user_dict.values())+128*4,1)
        self.liner4=nn.Linear(128+sum(user_dict.values())+128*4,1)
#         self.liner5=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
#         self.liner6=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
#         self.liner7=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
    def forward(self,userid,feedid,is_train=True):
        user_projections=[]
#         feed_projections=[]
        dense_embedding=[]
        sparse_embedding=[]
...
#         print(user_projections)
        ...
        spare_emb=spare_emb[feedid]
        dense_emb=self.dense_liner(torch.cat(dense_embedding,-1))
        dense_emb=dense_emb[feedid]
        text_emb=torch.cat(text_embedding+[self.feed_embed,self.graph],-1)
        text_emb=torch.cat([text_emb[feedid],self.user_graph[feedid]],-1)
        text_emb=self.text_liner(text_emb)  
        feed_feat=torch.cat([spare_emb,dense_emb,text_emb],-1) #128*4

        combine=torch.cat([user_feat[userid],feed_feat],axis=-1)
        ...
        logit_gnn5=self.liner1(torch.cat([outs[4],cross5],axis=-1))#+ffm3
        logit_gnn6=self.liner4(torch.cat([outs[5],cross6],axis=-1))#+ffm4
        logit_gnn7=self.liner4(torch.cat([outs[6],cross7],axis=-1))#+ffm4
...
class MMOELayer(nn.Module):
    def __init__(self, hidden_size, mmoe_hidden_dim=128,num_task=4,n_expert=3,expert_activation=None):
        ...
        # gates
        self.gates = [torch.nn.Parameter(torch.rand(hidden_size, n_expert).cuda(), requires_grad=True) for _ in range(num_task)]
        for gate in self.gates:
            gate.data.normal_(0, 1)
        self.gates_bias = [torch.nn.Parameter(torch.rand(n_expert).cuda(), requires_grad=True) for _ in range(num_task)]
        for i in range(num_task):
            setattr(self, 'task_{}_dnn'.format(i+1),DNN(mmoe_hidden_dim,(256,128),dropout_rate=0.3,l2_reg=0.5e-4,use_bn=True,seed=4096))
    def forward(self,x):
         # mmoe
...
def n_evaluate_nn(val_df,action_list,batch_size=4096*2):
    model.eval()
    leng=len(val_df)
...
def test_pred_func(model,test_df,test_src,test_dst,batch_size): 
