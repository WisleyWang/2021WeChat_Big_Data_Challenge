...
class Model3(nn.Module):
    def __init__(self,user_data,feed_data,textset,feed_embed,graph_emb):
        ...
        feed_dict={'bgm_song_id':33, 'bgm_singer_id':33,'authorid':33,'dense':32,'hash_dense':32
       ,'manual_keyword_id1':33,'manual_tag_id1':33,'machine_keyword_id1':33
            ,'machine_tag_id1':33,'knn_feed':33,
        ...
        self.spare_liner=nn.Linear(8*33,128)
        self.dense_liner=nn.Linear(32*2,128)
        self.text_liner=nn.Linear(32*6+512+64,256)
        ...
        self.cross1=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
        self.cross2=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
        self.cross3=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
        self.cross4=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
        self.cross5=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
        self.cross6=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
        self.cross7=CrossNetMix(sum(user_dict.values())+128*4+64,layer_num=4)
#         self.dnn=DNN(sum(user_dict.values())+128*3+64,(128,128),dropout_rate=0.1)
        self.mmoe=MMOELayer(sum(user_dict.values())+128*4+64, mmoe_hidden_dim=256,num_task=8,n_expert=8,expert_activation=None)

        self.liner1=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
        self.liner2=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
        self.liner3=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
        self.liner4=nn.Linear(128+sum(user_dict.values())+128*4+64,1)
...
#         print(user_projections)
        ...
        logit_reg=self.reg_liner(outs[7])
...
class MMOELayer(nn.Module):
    def __init__(self, hidden_size, mmoe_hidden_dim=128,num_task=4,n_expert=3,expert_activation=None):
        ...
        for i in range(num_task):
            setattr(self, 'task_{}_dnn'.format(i+1),DNN(mmoe_hidden_dim,(256,128),dropout_rate=0.4,l2_reg=1e-4,use_bn=True,seed=2022))
    def forward(self,x):
         # mmoe
