...
graph_emb=np.concatenate([np.load(ROOT_PATH+'tmp/grap_embedding32_hs2.npy'),np.load(ROOT_PATH+'tmp/grap_embedding32_sg2.npy')],axis=1)
...
model = Model3(user_data,feed_data,textset
...
optimizer, scheduler = build_optimizer(model, train_steps, learning_rate=2.4e-3)
all_pred=[]
...
#  保存模型
torch.save(model, ROOT_PATH+'/model/model3.pth')
#---------------------------------------------------
