...
batch_size=4096*3+2048
epochs=2
trn_dense=torch.from_numpy(train_ratings[feat].values).float()
#-------------------------------------------

model = Model1(user_data,feed_data,textset
...
optimizer, scheduler = build_optimizer(model, train_steps, learning_rate=3e-3)
all_pred=[]
...
#  保存模型
torch.save(model, ROOT_PATH+'/model/model1-1.pth')
#---------------------------------------------------
